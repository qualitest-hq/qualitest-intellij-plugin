...
- **HTTP 对接**：使用请求头 `X-Project-Token` 调用服务端接口 `/api/project/importApis`（项目上下文由服务端根据 Token 解析，无需在请求体里传 `testProjectId`）。请求体带 `uploadType`：`project`（项目级）/ `controllerAll` / `controllerSelect`；仅 `project` 会在项目鉴权配置为空时写入双端 Bearer 默认模板。
